...
<div class="beranda-page">
    <div class="hero">
        <div class="hero-left">
            <h1>
                Halo, <span>{{ $siswa->nama ?? $siswa->nis ?? 'Siswa' }}</span>.<br>
            ...
            <div class="hero-actions">
                <a href="{{ route('siswa.konseling.create') }}" class="cta-button">
    ...
    {{-- ===== ChatWidget (pojok kanan) — daftar chat dengan Guru BK ===== --}}
    <button type="button" class="chat-float-btn" id="chatFloatBtn" title="Buka pesan" aria-label="Buka pesan">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
        </svg>
        @if($chatSessions->count() > 0)
            <span class="chat-badge-float">{{ $chatSessions->count() > 9 ? '9+' : $chatSessions->count() }}</span>
        @endif
    </button>
    ...
    <div class="chat-widget" id="chatWidget">
        ...
        <div class="chat-list" id="tabChats">
            ...
            @else
                @foreach($chatSessions as $cs)
                    <a href="{{ route('siswa.chat', $cs->id) }}" class="chat-item" style="text-decoration:none;color:inherit">
                        <div class="chat-avatar">{{ strtoupper(mb_substr($cs->guru_bk ?? 'G', 0, 1)) }}</div>
                        <div class="chat-item-body">
                            <div class="chat-item-top">
                                <span class="chat-item-name">{{ $cs->guru_bk ?: 'Guru BK' }}</span>
                                <span class="chat-item-time">{{ $cs->tanggal ? \Carbon\Carbon::parse($cs->tanggal)->format('d M') : '' }}</span>
                            </div>
                            <div class="chat-item-preview">{{ \Illuminate\Support\Str::limit($cs->deskripsi ?? 'Konseling telah dikonfirmasi', 50) }}</div>
                            <div class="chat-item-meta">{{ $cs->kategori ?: 'Konseling' }} · Daring</div>
                        </div>
                    </a>
                @endforeach
            @endif
        ...
        <div class="contacts-list" id="tabContacts" style="display:none">
            <div class="empty-chat">
                ...
                <a href="{{ route('siswa.konseling.create') }}" class="cta-button" style="margin-top:12px;display:inline-flex">Pilih Guru BK</a>
            </div>
        </div>
    </div>

    {{-- ===== AI Chatbot FAB (pojok kiri) ===== --}}
...
(function () {
    ...
    var aiUrl = @json(route('siswa.ai.faq'));
