...
<head>
    ...
    <title>
        @if($role === 'siswa') Login Siswa
        @elseif($role === 'guru') Login Guru BK
        @elseif($role === 'kepsek') Login Kepala Sekolah
        @else Login Admin
        @endif
        — BK System
    </title>
...
<div class="auth-page theme-{{ $theme }}">
        ...
        <div class="auth-card">
                ...
                @if($role === 'siswa')
                    <div class="auth-field">
                        ...
                        <input
                            ...
                            class="{{ $errors->has('nis') || $errors->has('login') ? 'has-error' : '' }}"
                            required
                        >
                ...
                @else
                    <div class="auth-field">
                        ...
                        <input
                            ...
                            id="username"
                            name="username"
                            placeholder="{{ $role === 'guru' ? 'Contoh: joko_bk' : 'Username' }}"
                            value="{{ old('username') }}"
                            autocomplete="username"
                            class="{{ $errors->has('username') || $errors->has('login') ? 'has-error' : '' }}"
                            required
                        >
                    </div>
                @endif
            ...
            <div class="auth-switch">
                ...
                <div class="auth-switch-links">
                    @if($role !== 'siswa')
                        <a href="{{ route('login.role', 'siswa') }}" class="auth-switch-link">
                    ...
                    @endif
                    @if($role !== 'guru')
                        <a href="{{ route('login.role', 'guru') }}" class="auth-switch-link">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16">
                                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                                <circle cx="12" cy="7" r="4" />
                            </svg>
                    ...
                    @endif
                    @if($role !== 'kepsek')
                        <a href="{{ route('login.role', 'kepsek') }}" class="auth-switch-link">
                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16">
                                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                                <circle cx="12" cy="7" r="4" />
                            </svg>
