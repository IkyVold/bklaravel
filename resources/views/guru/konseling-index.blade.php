    ...
    <div class="container">
        ...
        <div class="guru-main">
            <div class="content-header">
                ...
                <p>Kelola, konfirmasi jadwal, dan pantau semua permintaan konseling dari siswa untuk <strong>{{ $namaGuru }}</strong></p>
            </div>

            <div class="content-tabs tab-nav">
                <a href="{{ route('guru.konseling.index', ['filter' => $currentFilter]) }}" class="tab-btn active">📋 Konseling</a>
            ...
            <div class="stats-row">
                ...
                <div class="stat-card">
                    ...
                    <div>
                        ...
                        <div class="stat-value">{{ $stats['terkonfirmasi'] }}</div>
                    </div>
                </div>
                <div class="stat-card">
            ...
            <div class="panel">
                <div class="panel-head">
                    <h3>{{ $filterTitles[$currentFilter] ?? 'Konseling' }}</h3>
                    <form method="GET" action="{{ route('guru.konseling.index') }}" class="panel-filters">
                        <input type="hidden" name="filter" value="{{ $currentFilter }}">
                        <input type="search" name="q" value="{{ $q }}" placeholder="🔍 Cari nama siswa...">
                    </form>

                        <a href="{{ route('guru.cetak', ['filter' => $currentFilter ?? 'all', 'q' => $q ?? '']) }}"
                           target="_blank"
                           class="btn-cetak"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="siswa-cell">
                                            <div class="siswa-av">{{ $av }}</div>
                                            <div>
                                                <strong>{{ $namaSiswa }}</strong>
                                                @if(!empty($row->pengajuan_sebelumnya_id))
                                                    <div class="badge-lanjutan">🔗 Lanjutan #{{ $row->pengajuan_sebelumnya_id }}</div>
                                                @endif
                                                @if(!empty($row->input_manual))
                                    ...
                                    <td><span class="badge-kelas">{{ $row->kelas_siswa ?? $siswa->kelas ?? '-' }}</span></td>
                                    <td>{{ $row->tanggal ? \Carbon\Carbon::parse($row->tanggal)->locale('id')->translatedFormat('d M Y') : '–' }}</td>
                                    <td>{{ $row->jam ? substr((string)$row->jam, 0, 5) : '–' }}</td>
                                    <td>{{ $row->tanggal_konfirmasi ? \Carbon\Carbon::parse($row->tanggal_konfirmasi)->locale('id')->translatedFormat('d M Y') : '–' }}</td>
                                    <td>{{ $row->jam_konfirmasi ? substr((string)$row->jam_konfirmasi, 0, 5) : '–' }}</td>
                                    <td>
                                        <span class="badge-jenis {{ $row->jenis === 'Daring' ? 'badge-daring' : 'badge-luring' }}">
                                            {{ $row->jenis === 'Daring' ? '🌐 Daring' : '🏫 ' . ($row->jenis ?: 'Luring') }}
                                        </span>
                                    </td>
                                    <td>{{ $row->kategori ?: '–' }}</td>
                                    <td class="deskripsi-cell" title="{{ $row->deskripsi }}">{{ \Illuminate\Support\Str::limit($row->deskripsi ?: 'Tidak ada deskripsi', 80) }}</td>
                                    <td>
                                        <span class="status-badge {{ $skLabel === 'Terkonfirmasi' ? 'status-selesai' : 'status-belum' }}">
                                            {{ $skLabel }}
                                        </span>
                                    </td>
                                    <td>
                                        <span class="status-badge {{ $status === 'Selesai' ? 'status-selesai' : ($status === 'Dibatalkan' ? 'status-dibatalkan' : 'status-proses') }}">
                                            {{ $status }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="aksi-wrap">
                                            <a href="{{ route('guru.konseling.show', $row->id) }}" class="btn-aksi btn-aksi-detail">📁 Detail</a>
                                            @if($belum)
                                                <a href="{{ route('guru.konseling.show', $row->id) }}" class="btn-aksi btn-aksi-konfirmasi">✅ Konfirmasi</a>
                                            @endif
                                            @if($sudahProses)
                                                <a href="{{ route('guru.konseling.show', $row->id) }}#laporan" class="btn-aksi btn-aksi-laporan">📝 Laporan</a>
                                            @endif
                                            @if($isOnline)
                                                <a href="{{ route('guru.chat', $row->id) }}" class="btn-aksi btn-aksi-chat">💬 Chat</a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
    ...
    <form method="POST" action="{{ route('guru.konseling.walkin.store') }}" style="padding:20px" id="formWalkin">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div>
          ...
          <input type="date" name="tanggal" value="{{ date('Y-m-d') }}" required style="width:100%;padding:10px 12px;border:.5px solid #d3d1c7;border-radius:8px;margin-bottom:10px;box-sizing:border-box">
        </div>
        <div>
          ...
          <select name="jam" required style="width:100%;padding:10px 12px;border:.5px solid #d3d1c7;border-radius:8px;margin-bottom:10px;box-sizing:border-box">
            @for($h=7;$h<=17;$h++)
              @foreach(['00','30'] as $m)
                @php $j = sprintf('%02d:%s',$h,$m); @endphp
...
(function(){
  ...
  @if(request('open')==='walkin') open(); @endif
  ...
  if(nis){
    nis.addEventListener('blur', function(){
      ...
      fetch(@json(url('/guru/siswa/lookup')) + '/' + encodeURIComponent(v), {
