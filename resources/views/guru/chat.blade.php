@extends('layouts.app')
@section('title', 'Chat · ' . ($siswaName ?? 'Siswa'))
@section('main_class', '')
@push('styles')
...
.chat-header-avatar { width:40px; height:40px; border-radius:10px; background:linear-gradient(135deg,#0F6E56,#1D9E75); color:#fff; display:flex; align-items:center; justify-content:center; font-weight:700; flex-shrink:0; }
...
@php
  ...
  $sendUrl = route('guru.chat.send', $row->id);
  $pollUrl = route('guru.chat.messages', $row->id);
  $backUrl = route('guru.konseling.show', $row->id);
@endphp
<div class="chat-shell">
  ...
  <div class="info-banner" id="infoBanner">✅ Chat aktif · Sesi dengan {{ $peerName }}</div>
  <div class="chat-messages" id="chatMessages">
    @forelse($messages as $m)
      @php $isSent = ($m->sender_type === 'guru') || ((string)$m->sender_id === $myId); @endphp
    ...
    @empty
      <div id="emptyHint" style="text-align:center;color:#888;padding:24px;font-size:13px">Belum ada pesan. Mulai percakapan dengan siswa.</div>
    @endforelse
  </div>
  <form class="chat-input-area" id="chatForm">
    @csrf
    <input type="text" id="chatInput" class="chat-input" placeholder="Tulis pesan ke siswa..." autocomplete="off" maxlength="2000">
